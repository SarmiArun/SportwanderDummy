...
export default function TenKm() {
  ...
  const men =
    Array.isArray(count) &&
    count.filter((d) => d.ErodeEvent !== null && d.categories === '10 km' && d.gender === 'male')
      .length;

  const women =
    Array.isArray(count) &&
    count.filter((d) => d.ErodeEvent !== null && d.categories === '10 km' && d.gender === 'female')
      .length;
